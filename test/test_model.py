...
def test_Sequential_exp_function():
    # f(x) = 1 - exp(-5x)
    m = SeqModel(
        in_features=1, depths=[1], out_features=1, act_func_type=ACT_FUNCT_TYPE.EXP
    )
    m[0].weight = nn.Parameter(torch.FloatTensor([[-5.0]]))
    m[0].bias = nn.Parameter(torch.FloatTensor([0.0]))
    m[2].weight = nn.Parameter(torch.FloatTensor([[-1.0]]))
    m[2].bias = nn.Parameter(torch.FloatTensor([1.0]))
    x = torch.linspace(0.0, 1.0, 101).reshape(-1, 1)
    x.requires_grad = True
    y_test = 1.0 - torch.exp(-5.0 * x)
    y_pred = m(x)
